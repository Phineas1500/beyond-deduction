...
class OntologyNode(object):
    ...
    def text(self):
        result = self.name + ";"
        if self.num_properties(None):
            visible_props = [str(prop) for prop in self.properties(True)]
            hidden_props = ["(" + str(prop) + ")" for prop in self.properties(False)]
            result += " prop:" + ', '.join(visible_props + hidden_props) + ";"
        if self.num_members(None):
            visible_members = [member for member in self.members(True)]
            hidden_members = ["(" + member + ")" for member in self.members(False)]
            result += " members:" + ', '.join(visible_members + hidden_members) + ";"
        return result
        ...
        for level_nodes in self.nodes:
            for node in level_nodes:
                ...
                if node.num_properties(False):
                    ...
                    # traverse along the path to find the LEAF node candidates
                    # (do NOT allocate directly to node - forces multi-hop reasoning)
                    candidate_nodes = [[node]]
                    while True:
                        next_candidate_nodes = []
                        for candidate_node in candidate_nodes[-1]:
                        ...
                        if not next_candidate_nodes:
                            break
                        candidate_nodes.append(next_candidate_nodes)
                    # Allocate ALL members to leaf nodes only
                    # This forces the model to traverse up the tree
                    for _ in range(Ontology.MIN_DISCRIMINANT + 1):
                        if not self.config.mix_hops:
                            choosen_node = choice(candidate_nodes[-1])
                        else:
                            # For mix_hops, still prefer deeper nodes but allow some variation
                            # Weight towards leaves to ensure multi-hop reasoning
                            choosen_node = choice(candidate_nodes[-1])
                        member = self.morphology.next_entity
                        choosen_node.add_member(member)
