# Modal deployment for Gemma 3 4B IT with vLLM
...
# Gemma 3 4B IT - instruction-tuned model
...
app = modal.App("gemma3-4b-inference")
...
@app.function(
    image=vllm_image,
    gpu=f"A100:{N_GPU}",  # 4B model fits easily on A100
...
async def test(test_timeout=10 * MINUTES):
    """Test the Gemma 3 4B endpoint."""
    import aiohttp

    url = serve.get_web_url()
    print(f"Gemma 3 4B server URL: {url}")
...
Only output final hypotheses."""
    ...
    async with aiohttp.ClientSession(base_url=url) as session:
        ...
        async with session.post("/v1/chat/completions", json=payload, headers=headers, timeout=2 * MINUTES) as resp:
            ...
            print(f"Gemma 3 4B response:\n{response}")


if __name__ == "__main__":
    print("Deploy with: modal deploy gemma3_4b_modal.py")
    print("Test with: modal run gemma3_4b_modal.py")
